...
(function($,window,document){
	$.fn.SwitchTabMulti=function(options){
		var defaults={
				...
				FixedBoxClass:'',//浮动菜单的class,
				FixedBoxClass2:'fixed2',//浮动菜单的class,
		...
		return this.each(function(i){
			...
			$objBox.find(".hd ul > li a").click(function(e){
				...
				if($objBox.find(".bd").outerHeight(true)>=$(window).height()){
					self.parents(".hd").addClass(opts.FixedBoxClass2);
				}
				var $target=$(self.attr("href"));
				var headerHeight=$(".hd").outerHeight(true);
				var targetOffset = $target.offset().top -  $objBox.find(".hd").outerHeight(true)+1;
				$objBox.find('.hd ul > li.'+dclasses.on).removeClass(dclasses.on);
				self.parent("li").addClass(dclasses.on);
				$('html, body').animate({
					"scrollTop": targetOffset
				}, opts.ScrollSpeed,opts.AnimateEasing ,function(){
					$objBox.find('.hd ul > li.'+dclasses.click).removeClass(dclasses.click);
				});
			})
			//滚动到对应位置置顶和选中
			$(document).on("scroll",function(){
				...
				if(FixedObjTop-ScrollTop<=headerHeight){
					$objBox.find(".hd").addClass(opts.FixedBoxClass2);
				}else{
					$objBox.find(".hd").removeClass(opts.FixedBoxClass2);
				}
